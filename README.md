...
- `docs/spec-v1.md`：v1 domain 規格摘要（命令、資料模型、JSON schema）
- `docs/pwa-plan.md`：PWA parity 設計（design-only）
- `docs/issue-checklist-v1.md`：完整里程碑 checklist 與目前完成度
- `domain/`：未來 shared domain module 位置（placeholder）
- `data/`：未來 Android Room / storage mapping 位置（placeholder）
- `ui/`：未來 Android Compose UI 位置（placeholder）
